...
text_history: str = _CURSOR_MARKER
...
class InputHistoryManager:
    ...
    def clear_input_history(self):
        self.text_history = _CURSOR_MARKER
        self.input_history = []
...
def move_cursor_right():
    global text_history
    items = text_history.split(_CURSOR_MARKER)
    if len(items) == 2:
        start = items[0]
        end_char = "" if len(items[1]) == 0 else items[1][0]
        end = "" if items[1] == 0 else items[1][:1]
        text_history = start + end_char + _CURSOR_MARKER + end

def move_cursor_left():
    global text_history
    items = text_history.split(_CURSOR_MARKER)
    if len(items) == 2:
        start = "" if len(items[0]) == 0 else items[0][:-1]
        start_char = "" if len(items[0]) == 0 else items[0][-1]
        end = "" if len(items[1]) == 0 else items[1]
        text_history = start + _CURSOR_MARKER + start_char + end

def append_input_history_event(text_insert: str, phrase: str = None):
    global input_history
    global text_history
    text_history = text_history.replace(cursor_position_marker, text_insert + cursor_position_marker)
