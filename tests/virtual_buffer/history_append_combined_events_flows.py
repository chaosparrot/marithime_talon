...
def test_full_self_repair_flow(assertion):
    ...
    input_history.add_event(InputEventType.INSERT, ["thisBigWord"], 1900)
    assertion( "    should only have one event appended", len(input_history.history) == 1)
    assertion( "    should only have the partial self repair added", input_history.get_last_event().type == InputEventType.SELF_REPAIR )
...
def test_correction_flow(assertion):
    ...
    input_history.add_event(InputEventType.INSERT, "thisBigWord", 1900)
    assertion( "    should only have one event appended", len(input_history.history) == 1)
    assertion( "    should only have the partial self repair added", input_history.get_last_event().type == InputEventType.CORRECTION )
