...
def test_track_selection_caret(assertion):
    ...
    keys = vb.navigate_to_token(vb.find_token_by_phrase("insert", next_occurrence=False), 0)
    ...
    vb.apply_key("shift:down right:5 shift:up")
    keys = vb.navigate_to_token(vb.find_token_by_phrase("sentence", next_occurrence=False), 0)
