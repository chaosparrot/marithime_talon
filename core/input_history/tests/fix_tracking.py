...
def test_single_token_half_context_replacement(assertion):
    ...
    assertion( input_fixer.known_fixes, False )
    #assertion( "        Should track the fix from 'that's' -> 'that' in the input fixer", input_fixer.get_key("that's", "that") in input_fixer.done_fixes)
