...
def test_detect_text_formatter(assertion):
    ...
    assertion( "    Should allow 'a' and ' ' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("Sugg", "sugg", "")
    current_token = VirtualBufferToken("abolition ", "abolition", "")
    assertion( "    Should allow 'Sugg' and 'abolition ' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("ending", "ending", "")
    current_token = VirtualBufferToken(" with", "with", "")
    assertion( "    Should not allow 'ending' and ' with' to be merged", can_merge_tokens(previous_token, current_token) == False)
    previous_token = VirtualBufferToken("ending \n", "ending", "")
    current_token = VirtualBufferToken("", "", "")
    assertion( "    Should not allow 'ending \n' and '' to be merged", can_merge_tokens(previous_token, current_token) == False)
    previous_token = VirtualBufferToken("", "", "")
    current_token = VirtualBufferToken("ending \n", "ending", "")
    assertion( "    Should allow '' and 'ending \n' to be merged", can_merge_tokens(previous_token, current_token))
...
def test_detect_snakecase_formatter(assertion):
    ...
    assertion( "    Should allow 'case' and '_' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("case_", "case", "snakecase")
    current_token = VirtualBufferToken("test", "", "snakecase")
    assertion( "    Should not allow 'case_' and 'test ' to be merged", can_merge_tokens(previous_token, current_token) == False)
    previous_token = VirtualBufferToken("case", "case", "snakecase")
    current_token = VirtualBufferToken("test_", "test", "snakecase")
    assertion( "    Should allow 'case' and 'test_' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("case", "case", "snakecase")
    current_token = VirtualBufferToken("_test", "test", "snakecase")
    assertion( "    Should allow not 'case' and '_test' to be merged", can_merge_tokens(previous_token, current_token) == False)
...
def test_detect_kebabcase_formatter(assertion):
    ...
    assertion( "    Should allow 'case' and '-' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("case-", "case", "kebabcase")
    current_token = VirtualBufferToken("test", "", "kebabcase")
    assertion( "    Should not allow 'case_' and 'test ' to be merged", can_merge_tokens(previous_token, current_token) == False)
    previous_token = VirtualBufferToken("case", "case", "kebabcase")
    current_token = VirtualBufferToken("test-", "test", "kebabcase")
    assertion( "    Should allow 'case' and 'test-' to be merged", can_merge_tokens(previous_token, current_token))
    previous_token = VirtualBufferToken("case", "case", "kebabcase")
    current_token = VirtualBufferToken("-test", "test", "kebabcase")
    assertion( "    Should allow not 'case' and '-test' to be merged", can_merge_tokens(previous_token, current_token) == False)
...
def test_detect_camelcase_formatter(assertion):
    ...
    assertion( "    Should not allow 'case' and 'Test' to be merged", can_merge_tokens(previous_token, current_token) == False)
    previous_token = VirtualBufferToken("case", "case", "camelcase")
