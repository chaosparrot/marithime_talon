...
def test_index_python_code(assertion):
    ...
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "    def change_place(test: str) -> False:")
...
def test_index_ruby_code(assertion):
    ...
    assertion("    should considered the second two tokens to be snakecase", len([token.format for token in sentence_tokens[2:4] if token.format == "snakecase"]) == 2)
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "    self.change_place(test:)")
...
def test_index_java_code(assertion):
    ...
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "    public static THIS_MACRO = 2;")
    ...
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "public class VirtualBufferIndexer extends VirtualBufferIndexerInterface {")
...
def test_index_php_code(assertion):
    ...
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "    public function sleep(float|int $seconds): void")
...
def test_index_javascript_code(assertion):
    ...
    assertion("    should index all characters", "".join([token.text for token in sentence_tokens]) == "window.localStorage.getItem(\"talon-practice-challengemode\") == \"true\";")
