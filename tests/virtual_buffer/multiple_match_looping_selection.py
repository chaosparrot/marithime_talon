...
def test_looping_new_from_end(assertion):
    ...
    vb.input_history.add_event(InputEventType.SELECT, ["new"])
...
def test_looping_adjacent_repetitions(assertion):
    vb = get_virtual_buffer()
    vb.insert_tokens(text_to_virtual_buffer_tokens("To ", "to"))
    vb.insert_tokens(text_to_virtual_buffer_tokens("to ", "to"))
    vb.insert_tokens(text_to_virtual_buffer_tokens("to", "to"))

    assertion( "    Moving from the end to the buffer to selecting the first occurrence of 'to'...") 
    vb.input_history.add_event(InputEventType.SELECT, ["to"])
    keys = vb.select_phrases(["to"])
    assertion( "        Should move to the left until we have reached the word 'to'", keys[0] == "left:2")
    assertion( "        Should move to the right until we have reached the end of the word 'to'", keys[1] == "shift-right:2")
    assertion( "    Repeating the search for 'to' again...")
    vb.input_history.add_event(InputEventType.SELECT, ["to"])    
    keys = vb.select_phrases(["to"])
    assertion( "        Should move to the left until we have reached the second word 'to'", keys[1] == "left:3")
    assertion( "        Should move to the right until we have reached the end of the word 'to'", keys[2] == "shift-right:3")
    assertion( "    Repeating the search for 'to' a third time...")
    vb.input_history.add_event(InputEventType.SELECT, ["to"])    
    keys = vb.select_phrases(["to"])
    assertion( "        Should move to the left until we have reached the second word 'to'", keys[1] == "left:3")
    assertion( "        Should move to the right until we have reached the end of the word 'to'", keys[2] == "shift-right:3")
    assertion( "    Repeating the search for 'to' a final time to loop back around...")
    vb.input_history.add_event(InputEventType.SELECT, ["to"])    
    keys = vb.select_phrases(["to"])
    assertion( "        Should move to the right until we have reached the first occurrence", keys[1] == "right:3")
    assertion( "        Should move to the right until we have reached the end of the word 'new'", keys[2] == "shift-right:2")
...
suite.add_test(test_looping_adjacent_repetitions)