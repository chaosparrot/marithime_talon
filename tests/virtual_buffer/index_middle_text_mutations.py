...
def test_removing_single_middle_of_token(assertion):
    ...
    assertion("    Should result the same amount of tokens as we started with", len(tokens) == len(starting_tokens))
    assertion("    Should not result in a clearing of the format of the starting tokens", tokens[0].format == "testingformat")
    assertion("    Should result in a reindexation", tokens[0].index_from_line_end != starting_tokens[0].index_from_line_end)
    assertion("    Should have the text 'tet.' at the end of the newly added tokens", "".join([token.text for token in tokens]).endswith("ist."))
...
def test_removing_multiple_middle_of_token(assertion):
    ...
    assertion("    Should result less tokens than those that we started with", len(tokens) < len(starting_tokens))
    assertion("    Should not result in a clearing of the format of the starting tokens", tokens[0].format == "testingformat")
    assertion("    Should result in a reindexation", tokens[0].index_from_line_end != starting_tokens[0].index_from_line_end)
...
def test_removing_multiple_start_of_token(assertion):
    ...
    assertion("    Should result in less tokens than those that we started with", len(tokens) < len(starting_tokens))
    assertion("    Should not result in a clearing of the format of the starting tokens", tokens[0].format == "testingformat")
    assertion("    Should result in a reindexation", tokens[0].index_from_line_end != starting_tokens[0].index_from_line_end)
