...
from ...virtual_buffer.typing import VirtualBufferTokenList, VirtualBufferMatch, VirtualBufferMatchVisitCache, SELECTION_THRESHOLD, CORRECTION_THRESHOLD
...
def get_multiple_query_word_match_tree_root(matcher: VirtualBufferMatcher, calculation, sublist, query_indices: List[int], buffer_index: int) -> VirtualBufferMatch:
    ...
    match_tree =  VirtualBufferMatch([query_indices], [[buffer_index]], query_words, [sublist.tokens[buffer_index].phrase], [score], max_score_per_word)
    match_tree.reduce_potential(max_score_per_word, score, summed_weights)
    return match_tree

def get_multiple_buffer_word_match_tree_root(matcher: VirtualBufferMatcher, calculation, sublist, query_index: int, buffer_indices: List[int]) -> VirtualBufferMatch:
    query_word = calculation.words[query_index]
...
def test_check_expand_backward(assertion):
    ...
    match_tree = get_single_word_match_tree_root(matcher, calculation, sublist, 0, 3)
    assertion("    should not be able to expand backward", match_tree.can_expand_backward(sublist) == False)

    assertion("Using the match 'incredible' on 'test with the incredibly good match'")
    calculation = matcher.generate_match_calculation(["an", "incredible"], select_threshold)
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    match_tree = get_single_word_match_tree_root(matcher, calculation, sublist, 1, 3)
    assertion("    should be able to expand backward", match_tree.can_expand_backward(sublist))
    match_trees, _ = matcher.expand_match_tree_backward(match_tree, calculation, sublist)
...
def test_check_expand_forward(assertion):
    ...
    calculation = matcher.generate_match_calculation(["an", "incredible"], select_threshold)
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    match_tree = get_single_word_match_tree_root(matcher, calculation, sublist, 0, 2)
    assertion("    should be able to expand forward", match_tree.can_expand_forward(calculation, sublist))
    match_trees, _ = matcher.expand_match_tree_forward(match_tree, calculation, sublist)
    assertion("    should have at least one result after expanding", len(match_trees) > 0)
    if len(match_trees) > 0:
        assertion("    should have a lower score potential than before", match_trees[0].score_potential < match_tree.score_potential)
        assertion("    should still have a score potential bigger than the threshold", match_trees[0].score_potential >= calculation.match_threshold)
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    match_tree = get_single_word_match_tree_root(matcher, calculation, sublist, 1, 3)
    assertion("    should not be able to expand forward", match_tree.can_expand_forward(calculation, sublist) == False)
...
def test_expand_skip_one_forward(assertion):
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    match_tree = get_single_word_match_tree_root(matcher, calculation, sublist, 0, 1)
    assertion("    should be able to expand forward", match_tree.can_expand_forward(calculation, sublist))
    match_trees, _ = matcher.expand_match_tree_forward(match_tree, calculation, sublist)
    assertion("    should have at least one result after expanding", len(match_trees) > 0)
    if len(match_trees) > 0:
...
def test_fully_combined_query_match_tree(assertion):
    ...
    match_tree = get_multiple_query_word_match_tree_root(matcher, calculation, sublist, [0, 1], 2)
    assertion("    should not be able to expand forward", match_tree.can_expand_forward(calculation, sublist) == False)
    assertion("    should not be able to expand backward", match_tree.can_expand_backward(sublist) == False)
    match_trees, _ = matcher.expand_match_tree(match_tree, calculation, sublist)
...
def test_partially_combined_query_match_tree(assertion):
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    calculation.cache.index_token_list(sublist)
    match_tree = get_multiple_query_word_match_tree_root(matcher, calculation, sublist, [0, 1], 3)
    assertion("    should be able to expand forward", match_tree.can_expand_forward(calculation, sublist))
    assertion("    should not be able to expand backward", match_tree.can_expand_backward(sublist) == False)
    match_trees, _ = matcher.expand_match_tree(match_tree, calculation, sublist)
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the incredibly good match"))
    match_tree = get_multiple_query_word_match_tree_root(matcher, calculation, sublist, [1, 2], 3)
    assertion("    should not be able to expand forward", match_tree.can_expand_forward(calculation, sublist) == False)
    assertion("    should be able to expand backward", match_tree.can_expand_backward(sublist))
    match_trees, _ = matcher.expand_match_tree(match_tree, calculation, sublist)
    assertion("    should have a single result after expanding", len(match_trees) == 1)
...
def test_fully_combined_buffer_match_tree(assertion):
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the in credible good match"))
    calculation.cache.index_token_list(sublist)
    match_tree = get_multiple_buffer_word_match_tree_root(matcher, calculation, sublist, 0, [3, 4])
    assertion("    should not be able to expand forward", match_tree.can_expand_forward(calculation, sublist) == False)
    assertion("    should not be able to expand backward", match_tree.can_expand_backward(sublist) == False)
    match_trees, _ = matcher.expand_match_tree(match_tree, calculation, sublist)
    assertion("    should have a single result after expanding", len(match_trees) == 1)
...
def test_partially_combined_buffer_match_tree(assertion):
    ...
    sublist = VirtualBufferTokenList(0, get_tokens_from_sentence("test with the in credible good match"))
    calculation.cache.index_token_list(sublist)
    match_tree = get_multiple_buffer_word_match_tree_root(matcher, calculation, sublist, 1, [3, 4])
    assertion("    should not be able to expand forward", match_tree.can_expand_forward(calculation, sublist) == False)
    assertion("    should be able to expand backward", match_tree.can_expand_backward(sublist))
    match_trees, _ = matcher.expand_match_tree(match_tree, calculation, sublist)
    assertion("    should have at least a single result after expanding", len(match_trees) >= 1)
